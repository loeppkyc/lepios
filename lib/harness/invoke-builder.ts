...
export type InvokeBuilderResult =
...
async function writeEvent(params: {
  ...
  try {
    ...
    await db.from('agent_events').insert({
      ...
      domain: 'orchestrator',
      action: 'invoke_builder',
      actor: 'harness',
      task_type: 'builder_invoked',
      status: params.status,
      output_summary: params.output_summary,
      meta: params.meta,
      tags: ['harness', 'invoke-builder'],
    })
  } catch {
...
export async function fireBuilder(params: {
  task_id: string
  run_id: string
}): Promise<InvokeBuilderResult> {
  const { task_id, run_id } = params
  // Trim to defend against trailing newlines in env (e.g. from Vercel CLI pull)
  const routineId = process.env.BUILDER_ROUTINE_ID?.trim()
  const routineToken = process.env.BUILDER_ROUTINE_TOKEN?.trim()

  if (!routineId || !routineToken) {
    await writeEvent({
      status: 'error',
      output_summary: 'BUILDER_ROUTINE_ID or BUILDER_ROUTINE_TOKEN not set',
      meta: { task_id, run_id, error: 'missing_env_vars' },
    })
    return {
      ok: false,
      error: 'BUILDER_ROUTINE_ID or BUILDER_ROUTINE_TOKEN not configured',
  ...
  await writeEvent({
    status: 'success',
    output_summary: `Builder invoked for task ${task_id.slice(0, 8)}, session ${fireData.claude_code_session_id}`,
    meta: {
      task_id,
      run_id,
  ...
  // Attribution: builder fired for this task
