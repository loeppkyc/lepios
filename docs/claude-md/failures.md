...
- **Fix/workaround:** F-N28-fix-A — removed all bash drain calls from coordinator.md; coordinator now saves `pending_notification_id` to `task_queue.metadata` via Supabase MCP and relies on pg_cron for delivery. Added startup “resume from pending notification” check that reads the saved row ID and polls Supabase directly — no Vercel curl needed.
