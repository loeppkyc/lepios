...
## S-N10 — search_path lockdown closed all 13 advisor findings; F-N7 regression guard (2026-05-06)

- **Win:** All 13 `function_search_path_mutable` WARN findings from the Supabase advisor closed in one session via three-tier rollout. Tier 1 (4 trivial trigger fns, PR #81) shipped first as a confidence builder. Tier 2 (8 plpgsql fns with table refs, PR #82) followed with per-fn smoke tests in a single sentinel-rollback transaction. Tier 3 (`match_knowledge`, the SQL function powering `/api/twin/ask`, PR #83) caught a parse-time issue: `<=>` operator from pgvector-in-public couldn't resolve with empty search_path, requiring `OPERATOR(public.<=>)` qualification. F-N7 architectural test (`tests/architecture/search-path-coverage.test.ts`, sibling of F-N6) blocks any future function from merging without the lockdown.
- **Why it worked:** Tiering by blast radius let the high-traffic function ship LAST, after the pattern was validated on lower-risk batches. Per-tier live smoke (DB-level for fns 1–12, end-to-end `/api/twin/ask` for fn 13) caught the OPERATOR-qualification gap on the FIRST apply attempt — error was `operator does not exist: public.vector <=> public.vector` — before any production traffic touched the broken function. The Tier 1+2 successes built confidence that the pattern was right; the Tier 3 wrinkle was specific to `LANGUAGE sql` (body parsed at CREATE-time vs plpgsql's runtime parse) + cross-schema operator resolution.
- **Pattern:** For any rule-set hardening (search_path, RLS, etc.) covering N functions/tables, tier by blast radius (trivial → moderate → high-traffic) and ship as separate PRs. Validate the pattern on cheap items first — when the high-traffic item surfaces a wrinkle, you've already got 12 successful applications confirming the rest of the approach. Pair every rule-set hardening with a static migration scan test (F-N#) so the next regression is caught at PR-time, not advisor-email-time. End-to-end smoke (HTTP request against the production endpoint) is materially stronger than DB-level smoke when the function is in the hot path of an API route — confirms the full call stack works, not just the SQL.
- **Reference:** PRs #81/#82/#83. Migrations 0128/0129/0130. `tests/architecture/search-path-coverage.test.ts`. Live advisor at session end: **0** `function_search_path_mutable` findings.
