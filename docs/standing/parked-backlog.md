...
| A4  | Internal status page — per-component uptime, 90-day history bars                 | [parked]    | /api/health exists; cockpit uptime history page not built                                                                      |
| A5  | AI Control Center — governance dashboard, agent audit trail, capability grant UI | [parked]    | Sub-features exist (/gpu-day, /local-ai, /failures) but unified governance page not shipped                                    |
...
| A9  | MileIQ CSV/PDF drive analyzer — CRA mileage deduction tool                       | [delivered] | app/api/mileage/import/route.ts — Claude parses MileIQ CSV, business-only filter, CRA km rate calc; /mileage cockpit page live |

## Tier B — Data & Infrastructure (9 items)

| ID  | Item                                                                          | State       | Notes                                                                                                                                    |
| --- | ----------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| B1  | Ollama Analyst System Prompt — qwen2.5:32b hardened against sycophancy        | [delivered] | lib/llm/prompts/analyst.md (full sycophancy rules) + lib/llm/ollama.ts (isSycophantic() detector, prompt loaded at init) — wired in main |
| B2  | Daily Gmail Scanner — receipts/invoices/statements foundation                 | [delivered] | app/api/cron/gmail-scan/route.ts — F17/F18 markers, statement arrival classifiers, full scan pipeline; lib/gmail/classifiers/ live       |
...
| B7  | CI/CD pipeline — GitHub Actions PR-level CI                                   | [parked]    | No .github/workflows. Currently ~20%: Vercel auto-deploy + husky pre-commit only                                                         |
...
| C3  | Personal Spending view — dedicated surface for personal accounts                   | [delivered] | app/(cockpit)/personal-expenses/ page shipped; task_queue #abac1bac was queued — feature is live                                                              |
...
| C11 | Cora's educational tracking                                                        | [delivered] | app/(cockpit)/coras-future/ page live — Grade 11/12/Post-secondary timeline, application statuses (upcoming/open/applied/accepted/missed/rejected), full CRUD |

## Tier D — Harness Automation, Queued Feature Class (6 items)

| ID  | Item                                                                 | State       | task_queue | Notes                                                                                                                                                           |
| --- | -------------------------------------------------------------------- | ----------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | harness_approval_listener — auto-fire builder on task_queue=approved | [parked]    | #57ef5c6a  | coordinator-resume handles notification approval loop, but auto-fire-builder on approved task is distinct — still awaiting_grounding                            |
| D2  | Watchdog monitor — detect silent stalls and alert                    | [delivered] | #ac4c57df  | lib/night_watchman/ (checks, repairs, detector, drafter, pr-opener, verifier) + /api/cron/night_watchman_scan + /api/harness/self-repair-tick — full stack live |
| D3  | Auto-approve acceptance docs when Twin confidence > 80%              | [parked]    | #b6055674  | lib/twin/query.ts has 0.80 threshold but wired to safety-arbitrate, not acceptance doc approval gate — queued, not built                                        |
| D4  | Auto-resume continuous mode when quota refreshes after halt          | [parked]    | #1fb70813  | quota-monitor handles halt; auto-resume on daily refresh not implemented — queued                                                                               |
...
_Mass sweep 2026-05-10: 6 items flipped to delivered (A9, B1, B2, C3, C11, D2). Evidence: code in main, grepped and verified. Score: 0.0 → 14.6._
