...
## Total Readiness: 67.4% (73.45 / 109 pts)
...
| B4  | Tool calling wired (Ollama → tools → results → model)               | 3      | 75% | 2.25         | **Wired 2026-05-04.** `app/api/chat/route.ts`: `streamText` called with `tools: buildTools(...)`, `toolChoice: 'auto'`, `stopWhen: stepCountIs(5)`. B4 acceptance tests added to `tests/api/chat.test.ts` (4 tests: tools present, all 11 names, toolChoice=auto, stopWhen wired). Remaining 25%: live E2E verify — Ollama actually invokes a tool during a real conversation (requires tunnel live).                                                                                                |
...
| #   | Line item                                                       | Weight | Pct | Contribution | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --- | --------------------------------------------------------------- | ------ | --- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | File read tool (read repo files from chat)                      | 3      | 85% | 2.55         | **Shipped (Chat UI Slice 6).** `lib/orb/tools/read-file.ts`: ALLOWED_PREFIXES (app/, lib/, components/, tests/, docs/, supabase/migrations/, public/, scripts/), DENY_PATTERNS (.env, secret, credential, .pem, .key, token), path traversal prevention, 8KB truncation. 9 security tests. Remaining 15%: E2E live verify with tunnel.                                                                                                                                                                                       |
...
| D3  | Database query tool (read LepiOS tables from chat)              | 4      | 85% | 3.40         | **Shipped (Chat UI Slice 6).** `lib/orb/tools/query-db.ts`: 9-table allowlist (agent_events, harness_components, task_queue, knowledge, utility_bills, mileage_trips, expenses, amazon_orders, harness_config). Filter + order_by + limit (max 20). Tests: dryRun gate in registry suite. Remaining 15%: E2E live verify.                                                                                                                                                                                                    |
| D4  | Harness submit tool (queue tasks from chat)                     | 4      | 85% | 3.40         | **Shipped (Chat UI Slice 6).** `lib/orb/tools/queue-task.ts`: dryRun=true default (preview), dryRun=false inserts to `task_queue`. `lib/orb/tools/submit-idea.ts` also wired. Tests: dryRun gate (2 cases) in `tests/orb/tools/send-telegram.test.ts`. Remaining 15%: E2E live verify.                                                                                                                                                                                                                                       |
...
| B — Brain / Model     | 20      | 14.25     | 71%       |
| C — Context / Memory  | 18      | 15.95     | 89%       |
| D — Tool Use          | 24      | 18.55     | 77%       |
| E — Identity / Polish | 7       | 3.40      | 49%       |
| F — Operational       | 15      | 1.10      | 7%        |
| **Total**             | **109** | **73.45** | **67.4%** |
...
2026-05-04 MDT / UTC — D2 file write tool + B4 tool calling wired. `lib/orb/tools/write-file.ts` (dryRun gate, path security, 32KB cap, 15 tests). B4: 4 tests verify all 11 tools passed to streamText + toolChoice=auto + stopWhen. Registry now 11 tools. D2 0→80%, B4 0→75%. Total: 62.4% → 67.4%.
