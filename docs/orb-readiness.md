...
## Total Readiness: 72.3% (78.85 / 109 pts)
...
| D2  | File write tool with diff approval gate                         | 4      | 80%  | 3.20         | **Shipped 2026-05-04.** `lib/orb/tools/write-file.ts`: same path security as readFileTool (ALLOWED_PREFIXES + DENY_PATTERNS), dryRun=true default (preview: path, content, current_lines, new_lines), dryRun=false writes to disk via `fs.writeFileSync`. 32KB content cap. Creates parent dirs. 15 tests: dryRun gate (3), actual write (3), path security (6), size guard (2), capability (1). Remaining 20%: E2E live verify + AI SDK `needsApproval` UI gate.                                                            |
...
| D7  | Tool use bridge — AI SDK `ToolLoopAgent` + `needsApproval` gate | 4      | 90%  | 3.60         | **Fully wired (Chat UI Slice 6).** `lib/orb/tools/registry.ts`: 10 tools registered, `buildTools()` wires capability check (via `checkCapability`), 30s timeout, `agent_events` logging. `streamText` with `tools: buildTools(...)` + `stopWhen: stepCountIs(5)` in `/api/chat/route.ts`. Approval gate: dryRun pattern (action tools default dryRun:true). Tests: registry shape, capability gate, denied path, successful execute, dryRun gate (28 new tests). Remaining 10%: AI SDK `needsApproval` UI-level integration. |
|     | **Category total**                                              | **24** |      | **20.20**    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
