...
| —    | `safety-agent`           | Pre-flight safety review         | 95  | 8      | 40       | [T-002](leverage-targets.md#safety-agent-0--done)                               | Sub-phases A–E shipped 2026-05-08. Migration 0162 + 5 signals + scorer + router + E2E + twin arbiter + driver + F-N10 fix (worktree-strict claim) + 3-PR integration test (trivial typo / additive feature with scope creep / secret leak — all routed correctly per spec) + 191 tests. Deferred to v2.1: deploy-gate-runner wire-up (one-line driver call); morning_digest line + /autonomous counter (surfacing layer). |
...
| 6    | `coordinator-agent`      | Sprint coordinator               | 100 | 9      | 0        | [T-001](leverage-targets.md#t-001--coordinator-v1-remote-invocation)            | T-001 shipped 2026-05-08. Migration 0163 + /api/coordinator/fire + /api/coordinator/complete + halt check in pickup-runner + Telegram /run /queue /halt /resume + webhook routing + /coordinator cockpit page + queue counter on /autonomous. Loop-to-next live. 25 tests.                                                                                                                                                |
...
| 12   | `retail-scout-arbitrage` | Retail Scout / Arbitrage         | 72  | 7      | 196      | `docs/sprint-7/retail-arb-engine-acceptance.md`                                 | ~72% ported (Flipp deals, Brand Risk, ROI Calc, StockTrack, Auto Scan, Keepa lookup). Missing: Arb Engine tab (in-build, task 3a13fc07), Dashboard store cards (S), Purchase History (S), Price URL monitor (M). Updated 2026-05-15 per coordinator audit. |
...
| `harness-coordinator-agent` | Sprint coordinator                                   | shipped | 100 | 9      | 0        | `.claude/agents/coordinator.md`, T-001 shipped 2026-05-08, loop-to-next live                                                                                             |
...
| `harness-safety-agent`      | Six-signal risk score + Twin arbiter + Puppeteer E2E | shipped | 100 | 8      | 0        | v2.1 shipped 2026-05-08. Gate-runner wire-up + fetchPRDiffInput + morning_digest line + /autonomous counters + gate-wire tests. Safety Agent now gates all prod deploys. |
