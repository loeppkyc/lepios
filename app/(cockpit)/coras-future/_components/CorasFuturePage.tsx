...
const STATUSES: CoraFutureItem['status'][] = ['upcoming', 'open', 'applied', 'accepted', 'missed', 'rejected']
...
function DeadlinePill({ datesText }: { datesText: string | null }) {
  ...
  if (days < 90) return <span className="rounded-full bg-red-500/20 px-2 py-0.5 text-xs text-red-300">{days}d</span>
  if (days < 365) return <span className="rounded-full bg-yellow-500/20 px-2 py-0.5 text-xs text-yellow-300">{days}d</span>
  return <span className="rounded-full bg-green-500/20 px-2 py-0.5 text-xs text-green-300">{days}d</span>
}
...
function ItemCard({ item, onStatusChange }: {
  ...
  return (
    <div className="border-border rounded-md border">
      ...
      {expanded && (
        <div className="border-t border-border bg-[var(--color-cockpit-surface-2)] px-4 py-3 space-y-3">
...
function AddItemForm({ onAdded }: { onAdded: (item: CoraFutureItem) => void }) {
  ...
  const [form, setForm] = useState({
    ...
    name: '', provider: '', eligibility: '', value: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    onAdded(item)
    setForm({ category: 'program', name: '', provider: '', eligibility: '', value: '', timeline: '', dates: '', url: '', status: 'upcoming', notes: '' })
    setSaving(false)
    setOpen(false)
  }
  ...
  return (
    <div className="mt-4">
      ...
      {open && (
        <form onSubmit={handleSubmit} className="border-border mt-2 rounded-md border p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Type', field: 'category' as const, type: 'select', options: ['program', 'scholarship', 'note'] },
...
export function CorasFuturePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <h1 className="mb-1 text-2xl font-bold text-[var(--color-text-primary)]">Cora&apos;s Future</h1>
      <p className="mb-6 text-sm text-[var(--color-text-secondary)]">Programs, scholarships & planning for Cora&apos;s path forward</p>
      ...
      {activeTab === 'Programs & Scholarships' && (
        <div className="space-y-6">
          {TIMELINES.map((tl) =>
            grouped[tl].length > 0 ? (
              <div key={tl}>
                <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary)]">{tl}</p>
          ...
          {noTimeline.length > 0 && (
            <div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary)]">Other</p>
              <div className="space-y-2">
                {noTimeline.map((item) => (
                  <ItemCard key={item.id} item={item} onStatusChange={handleStatusChange} />
                ))}
          ...
          {programs.length === 0 && (
            <p className="text-sm text-[var(--color-text-secondary)]">No programs tracked yet. Add one below.</p>
          )}
            ...
            <div className="space-y-2">
              {withDates.map((item) => (
                <div key={item.id} className="border-border rounded-md border px-4 py-3">
                  ...
                  <p className="mt-1 text-xs text-[var(--color-text-secondary)]">{item.dates} · {item.provider}</p>
                  {item.notes && <p className="mt-1 text-xs text-[var(--color-text-secondary)]">{item.notes.slice(0, 120)}</p>}
                </div>
              ))}
            </div>
          )}
          <div className="border-border mt-6 rounded-md border bg-[var(--color-cockpit-surface)] p-4">
            <p className="mb-2 text-xs font-semibold text-[var(--color-text-primary)]">Quick Reference</p>
            <div className="space-y-1 text-xs text-[var(--color-text-secondary)]">
              <p><span className="text-[var(--color-text-primary)]">Current Grade:</span> ~Grade 6 (age 11)</p>
              ...
              <p className="mt-2"><span className="text-[var(--color-text-primary)]">First Nations Status:</span> Eligible for Indigenous-specific subsidies and scholarships.</p>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-3">
              {notes.map((item) => (
                <div key={item.id} className="border-border rounded-md border p-4">
                  ...
                  <p className="mt-2 text-xs text-[var(--color-text-secondary)]">Added {new Date(item.created_at).toLocaleDateString('en-CA')}</p>
                </div>
              ))}
            </div>
          )}
      ...
      {activeTab === "Cora's World" && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            {[
              { title: 'Godot 3D Game', desc: 'Zelda BOTW inspired, NPC souls, health tracking', code: 'coras-world/' },
              { title: 'React Web App', desc: 'Room builder, world explorer, vibes tracker', code: 'coras-world-app/' },
              { title: 'Streamlit App', desc: 'Mood tracking, math arena, style studio', code: 'coras_world/' },
            ].map((p) => (
              <div key={p.title} className="border-border rounded-md border p-4">
                <p className="mb-1 text-sm font-semibold text-[var(--color-text-primary)]">{p.title}</p>
                <p className="mb-2 text-xs text-[var(--color-text-secondary)]">{p.desc}</p>
                <code className="rounded bg-[var(--color-cockpit-surface-2)] px-2 py-0.5 text-xs text-[var(--color-accent-gold)]">{p.code}</code>
              </div>
            ))}
          </div>
          <div className="border-border rounded-md border p-4">
            <p className="mb-2 text-sm font-semibold text-[var(--color-text-primary)]">Health & Wellness</p>
