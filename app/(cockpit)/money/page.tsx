...
// Sprint 4: extract to components/cockpit/ArcGauge.tsx
function ArcGauge({ value, label, size = 120 }: { value: number; label: string; size?: number }) {
  const cx = size / 2,
    cy = size / 2 + 5,
    r = size * 0.38
...
export default async function MoneyPage() {
  ...
  const completed30 = allBets.filter((b) => ['win', 'loss', 'push'].includes(b.result)).slice(0, 30)
  ...
  const { data: deals, error } = await supabase
    .from('deals')
    .select(
      'id, asin, title, product_type, source, sell_price_cad, roi_pct, sales_rank, status, found_date'
    )
    .eq('status', 'found')
  ...
  const avgRoi =
    dealCount > 0 ? Math.round(deals!.reduce((s, d) => s + (d.roi_pct ?? 0), 0) / dealCount) : 0
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--color-base)', padding: '24px' }}>
      ...
      {/* ── Command Center — Sprint 7 Chunk C ────────────────────────── */}
      <CommandCenter />
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            {dealCount > 0 && (
              <>
                ...
                >
                  top ROI:{' '}
                  <span style={{ color: 'var(--color-positive)' }}>{topRoi?.toFixed(0)}%</span>
                </span>
                <span
                  style={{
                    fontFamily: 'var(--font-mono)',
            ...
            {deals!.map((deal, i) => (
              <div
                ...
                style={{
                  ...
                  padding: '10px 20px',
                  borderBottom:
                    i < deals!.length - 1 ? '1px solid var(--color-border-pillar)' : undefined,
              ...
              >
                ...
                <span
                  style={{
                    ...
                    textTransform: 'uppercase',
                    color:
                      deal.product_type === 'lego'
                        ? 'var(--color-pillar-growing)'
                        : deal.product_type === 'book'
                          ? 'var(--color-pillar-happy)'
