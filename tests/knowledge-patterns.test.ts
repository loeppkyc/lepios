...
function makeFailure(
  ...
  return {
    id,
    occurred_at: opts.occurredAt ?? `2026-04-${String(Number(id.slice(-2)) || 1).padStart(2, '0')}T02:00:00Z`,
...
describe('analyzeFailedApproaches — aggregation', () => {
  ...
  it('candidate uses the most-recent failure error_message (last in array)', () => {
    const events: AgentEventRow[] = [
      ...
      makeFailure('evt-3', { occurredAt: '2026-04-03T01:00:00Z', errorMessage: 'most recent message' }),
    ]
