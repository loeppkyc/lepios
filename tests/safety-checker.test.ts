...
// ── Mock logEvent (knowledge client) ─────────────────────────────────────────

vi.mock('@/lib/knowledge/client', () => ({
  logEvent: vi.fn(),
}))

import { runSafetyChecks, validateProposedChanges } from '@/lib/safety/checker'
...
describe('Rule 1 — destructive SQL', () => {
  ...
  it('flags DROP COLUMN as critical', () => {
    const report = runSafetyChecks(
      cleanInput({
        migrations: [
          { name: '0099_test', sql: 'ALTER TABLE bets DROP COLUMN old_col;', hasRollback: false },
        ],
      })
    )
  ...
  it('does NOT flag ALTER TABLE ADD COLUMN', () => {
    const report = runSafetyChecks(
      cleanInput({
        migrations: [
          { name: '0099_test', sql: 'ALTER TABLE bets ADD COLUMN notes TEXT;', hasRollback: true },
        ],
      })
    )
    expect(report.checks.filter((c) => c.category === 'destructive_operation')).toHaveLength(0)
...
describe('Rule 2 — secret leak', () => {
  it('flags hardcoded AWS access key', () => {
    ...
    expect(report.checks.find((c) => c.id === 'secret_leak_aws_access_key')?.severity).toBe(
      'critical'
    )
  })
...
describe('Rule 3 — missing test coverage', () => {
  it('flags new lib file without corresponding test', () => {
    const report = runSafetyChecks(
      cleanInput({
        fileChanges: [
          { path: 'lib/knowledge/client.ts', diff: 'export function foo() {}', isNew: true },
        ],
      }),
      NO_TESTS
    )
  ...
  it('flags new api route file without test', () => {
    const report = runSafetyChecks(
      cleanInput({
        fileChanges: [
          { path: 'app/api/foo/route.ts', diff: 'export async function GET() {}', isNew: true },
        ],
      }),
      NO_TESTS
    )
  ...
  it('does NOT flag non-lib non-api files', () => {
    const report = runSafetyChecks(
      cleanInput({
        fileChanges: [
          { path: 'components/foo.tsx', diff: 'export function Foo() {}', isNew: true },
        ],
      }),
      NO_TESTS
    )
...
describe('Rule 5 — missing rollback', () => {
  it('flags migration with no rollback comment', () => {
    const report = runSafetyChecks(
      cleanInput({
        migrations: [
          { name: '0099_test', sql: 'CREATE TABLE foo (id UUID PRIMARY KEY);', hasRollback: false },
        ],
      })
    )
  ...
  it('does NOT flag when hasRollback=true', () => {
    const report = runSafetyChecks(
      cleanInput({
        migrations: [
          { name: '0099_test', sql: 'CREATE TABLE foo (id UUID PRIMARY KEY);', hasRollback: true },
        ],
      })
    )
        ...
        fileChanges: [
          {
            ...
            diff: 'export async function POST(req) { const body = await req.json() }',
            isNew: true,
          },
        ],
  ...
  it('does NOT flag nightly cron route (body-exempt)', () => {
    const report = runSafetyChecks(
      cleanInput({
        fileChanges: [
          {
            path: 'app/api/knowledge/nightly/route.ts',
...
// ── validateProposedChanges — capability gate ─────────────────────────────────

describe('validateProposedChanges — capability gate', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('calls requireCapability with shell.run and default agentId coordinator', async () => {
    await validateProposedChanges(cleanInput())
  ...
  it('passes custom agentId to requireCapability', async () => {
    await validateProposedChanges(cleanInput(), undefined, { agentId: 'builder' })
  ...
  it('re-throws CapabilityDeniedError when capability is denied', async () => {
    ...
    await expect(validateProposedChanges(cleanInput())).rejects.toThrow(CapabilityDeniedError)
  })

  it('does not run safety checks when capability is denied', async () => {
    mockRequireCapability.mockRejectedValue(
      new CapabilityDeniedError('coordinator', 'shell.run', 'no_grant_for_agent')
    )

    // If runSafetyChecks ran, it would return a report — we should get a rejection instead
    const result = await validateProposedChanges(cleanInput()).catch((e: unknown) => e)
    expect(result).toBeInstanceOf(CapabilityDeniedError)
  })

  it('returns a SafetyReport when capability is allowed', async () => {
    const result = await validateProposedChanges(cleanInput())
