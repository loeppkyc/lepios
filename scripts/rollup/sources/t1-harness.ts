...
export async function computeT1(): Promise<TrackResult> {
  const t0 = Date.now()
  try {
    const db = createServiceClient()
    const { data, error } = await db.from('harness_components').select('weight_pct, completion_pct')

    if (error) throw new Error(error.message)
    if (!data || data.length === 0) throw new Error('harness_components returned 0 rows')
