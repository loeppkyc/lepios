CREATE TABLE IF NOT EXISTS price_snapshots (
  ...
  snapped_date DATE NOT NULL DEFAULT CURRENT_DATE  -- plain date column for immutable unique index
);

-- One snapshot per ASIN per price_type per day (dedup)
-- Uses snapped_date (DATE) instead of snapped_at::date — TIMESTAMPTZ casts are not IMMUTABLE
CREATE UNIQUE INDEX IF NOT EXISTS idx_ps_asin_type_day
  ON price_snapshots(asin, domain, price_type, snapped_date);
